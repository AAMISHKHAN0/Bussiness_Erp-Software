...
export default function SalesReportModal({ isOpen, onClose, orders = [], customers = [] }) {
  ...
  const handleExportCSV = () => {
    const headers = ['Order Number', 'Date', 'Customer', 'Payment Terms', 'Payment Status', 'Fulfillment Status', 'Net Amount ($)'];
      ...
      <div className="space-y-6 text-slate-900 print:text-black">
        ...
        <div className="flex items-center justify-between gap-3 print:hidden bg-slate-50 p-3 rounded-xl border border-slate-200">
          <p className="text-xs text-slate-600 font-medium">
            Fiscal Year 2026 Audit Manifest · Generated on {new Date().toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}
          </p>
          <div className="flex items-center gap-2">
            <button
              onClick={handleExportCSV}
              className="flex items-center gap-1.5 px-3 py-1.5 bg-white hover:bg-slate-100 border border-slate-200 text-slate-700 text-xs font-semibold rounded-lg shadow-2xs transition-colors"
            >
            ...
            <div>
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-500 font-medium">Financial & Commercial Sales Division</p>
                </div>
              </div>
              <p className="text-xs text-slate-600 mt-2 leading-relaxed">
                450 Lexington Avenue, 28th Floor, New York, NY 10017<br />
                EIN: US-12-3456789 · GAAP Compliant Sales Ledger
              </p>
            </div>

            <div className="text-right">
              <span className="text-xl font-black text-blue-600 tracking-tight uppercase">SALES REPORT</span>
              <p className="text-xs font-bold text-slate-700 mt-1">Ref: SR-{new Date().getFullYear()}-{orders.length}</p>
          ...
          <div className="grid grid-cols-4 gap-3 py-2">
            <div className="p-3.5 rounded-lg border border-slate-200 bg-slate-50/70">
              ...
              <p className="text-lg font-extrabold text-slate-900 mt-0.5">
                ${totalSalesVolume.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
              </p>
                ...
                {orders.map((o) => (
                  <tr key={o.id} className="hover:bg-slate-50/50">
                    ...
                    <td className="p-2.5 text-right font-mono font-bold text-slate-900">
                      ${parseFloat(o.net_amount || o.total_amount || 0).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                    </td>
                  </tr>
                ))}
              </tbody>
          ...
          <div className="pt-6 border-t border-slate-200 grid grid-cols-2 gap-8 text-xs text-slate-500">
            <div>
              ...
              <div className="border-b border-slate-300 pb-4 mb-1">
                <span className="font-mono text-slate-800 text-[11px]">Victoria Chen, CPA — Financial Controller</span>
              </div>
            ...
            <div>
              ...
              <div className="border-b border-slate-300 pb-4 mb-1">
                <span className="font-mono text-slate-800 text-[11px]">Derrick Cole — VP Commercial Accounts</span>
              </div>
              <p className="text-[10px]">Confidential — Internal & Client Audit Purposes Only</p>
            </div>
          </div>
        </div>
      </div>
